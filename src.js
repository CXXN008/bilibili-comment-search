...
if (window.danmaku === undefined) {
  dmLoad().then(() => {
    if ($(`#dm-query`).length === 0) {
      ...
      d.innerHTML = `<style>.bi-btn{display:inline-block;background:#00a1d6;color:#fff;font-size:14px;padding:4px 18px;border-radius:4px;border-top-left-radius:4px;border-top-right-radius:4px;border-bottom-right-radius:4px;border-bottom-left-radius:4px;transition:all.3s;transition-property:all;transition-duration:0.3s;transition-timing-function:initial;transition-delay:initial;user-select:none;border:1px solid #00a1d6;border-top-color:rgb(0,161,214);border-top-style:solid;border-top-width:1px;border-right-color:rgb(0,161,214);border-right-style:solid;border-right-width:1px;border-bottom-color:rgb(0,161,214);border-bottom-style:solid;border-bottom-width:1px;border-left-color:rgb(0,161,214);border-left-style:solid;border-left-width:1px;border-image-source:initial;border-image-slice:initial;border-image-width:initial;border-image-outset:initial;border-image-repeat:initial;text-align:center;cursor:pointer;}</style><div style="margin:1rem" id="dm-query" onclick="searchDm()" class="bi-btn button" role="button"><span class="bpui-button-text"> 搜 索 </span></div><input id="dm-key" type="text" onkeydown='if(event.keyCode==13) searchDm()' placeholder="关键字" style="height:1.5rem;width:12rem;margin:1rem"><span id="dm-count">Data ready!</span><div style="margin:1rem" id="dm-sort" onclick="dmSort()" class="bi-btn button" role="button"><span class="bpui-button-text">视频时序</span></div><div style="margin:1rem" onclick="dmLoad().then(() => {$('#dm-list>li').remove();$('#dm-sort')[0].innerText='视频时序';$('#dm-count')[0].innerText='';window.oList=undefined})" class="bi-btn button" role="button"><span class="bpui-button-text">重载弹幕</span></div><div style="display:inline-block;width:100%;overflow:hidden;" id="dm-list"></div>`
